...
| **13 — Realtime source seam** (2–3 wks) | `superplayer-realtime`: a public `FrameSource` a transport implements — encoded frames with timestamps, codec and optional codec-specific data, naming no Media3 type — and behind it the push `MediaSource`/`MediaPeriod` writing into Media3 `SampleQueue`s under a live, unseekable timeline, the codec-string mapping, the codec-specific-data conversion and its in-band alternative, and a conformance suite in testkit. | A `FrameSource` fake plays synthetic H.264 and AAC in sync to `STATE_READY` with an advancing position under Robolectric, with no device and no network; `FrameSourceConformance` passes over the fake; a golden trace is committed. |
