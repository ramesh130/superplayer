...
| [0018 — Push encoded frames through one `FrameSource` into Media3's own sample queues](0018-push-encoded-frames-through-one-framesource-into-media3s-own-sample-queues.md) | A realtime transport hands over encoded frames through one `FrameSource` naming no Media3 type, and an internal `MediaSource`/`MediaPeriod` writes them into Media3's own `SampleQueue`s under a live unseekable timeline, so Media3 keeps decoding and the TV path, secure decoders and tunneling are not rewritten; container framing is stripped before the seam and codec-specific data is keyed on the codec's fourcc rather than the container; and what the path does **not** reach — the cache, CMCD, downloads, the load-error rungs and bandwidth estimation — is written down rather than discovered, with signalling deliberately kept off `HttpTransport`. |

Each record states its own status in its header. **All eighteen are Accepted**, and nothing has
