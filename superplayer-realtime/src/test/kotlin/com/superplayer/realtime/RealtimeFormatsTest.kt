...
class RealtimeFormatsTest {
    ...
    fun `h264 is mapped under both of its sample entry names`() {
        assertThat(RealtimeFormats.formatFor("avc1.640028").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H264)
        assertThat(RealtimeFormats.formatFor("avc3.42E01E").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H264)
    }

    @Test
    fun `h265 is mapped under both of its sample entry names`() {
        assertThat(RealtimeFormats.formatFor("hvc1.1.6.L93.B0").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H265)
        assertThat(RealtimeFormats.formatFor("hev1.1.6.L93.B0").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H265)
    }

    @Test
    fun `vp9 av1 aac and opus are mapped`() {
        assertThat(RealtimeFormats.formatFor("vp09.00.10.08").sampleMimeType).isEqualTo(MimeTypes.VIDEO_VP9)
        assertThat(RealtimeFormats.formatFor("av01.0.04M.08").sampleMimeType).isEqualTo(MimeTypes.VIDEO_AV1)
        assertThat(RealtimeFormats.formatFor("mp4a.40.2").sampleMimeType).isEqualTo(MimeTypes.AUDIO_AAC)
        assertThat(RealtimeFormats.formatFor("opus").sampleMimeType).isEqualTo(MimeTypes.AUDIO_OPUS)
    }
    ...
    fun `mpeg-2 aac reaches the same mime type as mpeg-4 aac`() {
        ...
        assertThat(RealtimeFormats.formatFor("mp4a.67.2").sampleMimeType).isEqualTo(MimeTypes.AUDIO_AAC)
    }
    ...
    fun `sdp encoding names reach the same table as the rfc 6381 strings`() {
        ...
        assertThat(RealtimeFormats.formatFor("H264").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H264)
        assertThat(RealtimeFormats.formatFor("H265").sampleMimeType).isEqualTo(MimeTypes.VIDEO_H265)
        assertThat(RealtimeFormats.formatFor("VP9").sampleMimeType).isEqualTo(MimeTypes.VIDEO_VP9)
        assertThat(RealtimeFormats.formatFor("AV1").sampleMimeType).isEqualTo(MimeTypes.VIDEO_AV1)
        assertThat(RealtimeFormats.formatFor("MPEG4-GENERIC").sampleMimeType).isEqualTo(MimeTypes.AUDIO_AAC)
        assertThat(RealtimeFormats.formatFor("opus").sampleMimeType).isEqualTo(MimeTypes.AUDIO_OPUS)
    }
    ...
    fun `an sdp encoding name states no rfc 6381 codecs string`() {
        ...
        assertThat(RealtimeFormats.formatFor("H264").codecs).isNull()
        assertThat(RealtimeFormats.formatFor("avc1.640028").codecs).isEqualTo("avc1.640028")
    }
    ...
    fun `the strings observed in the moq spike map and carry their profile and level`() {
        ...
        for (codec in listOf("avc1.42c01e", "avc3.42c01e")) {
            assertThat(RealtimeFormats.formatFor(codec).sampleMimeType).isEqualTo(MimeTypes.VIDEO_H264)
    ...
    fun `an unrecognised codec string refuses and names itself`() {
        val refusal = assertThrows(UnsupportedRealtimeCodecException::class.java) {
            RealtimeFormats.formatFor("theora")
        }
    ...
    fun `a mapped family with malformed parameters refuses rather than dropping them`() {
        ...
        assertThrows(UnsupportedRealtimeCodecException::class.java) { RealtimeFormats.formatFor("avc1.zzzzzz") }
        assertThrows(UnsupportedRealtimeCodecException::class.java) { RealtimeFormats.formatFor("avc1") }
        assertThrows(UnsupportedRealtimeCodecException::class.java) { RealtimeFormats.formatFor("mp4a.40") }
    }
    ...
    fun `an mp4a string that is not aac refuses rather than being decoded as aac`() {
        ...
        val refusal = assertThrows(UnsupportedRealtimeCodecException::class.java) {
            RealtimeFormats.formatFor("mp4a.69.2")
        }
    ...
    fun `a registered fourcc this table does not claim refuses`() {
        ...
        assertThrows(UnsupportedRealtimeCodecException::class.java) { RealtimeFormats.formatFor("avc2.42c01e") }
    }

    @Test
    fun `an empty codec string refuses`() {
        val refusal = assertThrows(UnsupportedRealtimeCodecException::class.java) { RealtimeFormats.formatFor("") }
