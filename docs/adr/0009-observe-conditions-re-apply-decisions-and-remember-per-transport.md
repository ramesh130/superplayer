...
- **Refined by:** [ADR-0014](0014-match-the-display-and-watch-it-change-behind-one-output-slot.md)
  rules 5 and 7, recorded as an addendum at rule 5, and rules 9 to 11, which change the kind of rule
  2's display row and are recorded there. (ADR-0010 and ADR-0011 refine rule 7 and are recorded at
  it.)
...
2. **The observations named by `PRD.md` §3.1 and #96 that Phase 3 does not add are these, and each
   ...
   *Addendum (2026-09-17, ADR-0014 rules 9 to 11).* The first row has arrived, and it has changed
   kind. On a television the display changes under a playing session: an HDMI hotplug, an AV
   receiver in the chain, a lesser panel swapped in. By this rule's own test it is therefore no longer
   a constraint. It becomes a *live reading* rather than an observation: a `DisplayCapability` that
   core re-reads on a change, on a player built with `superplayer-tv`, into a `DisplayInForce` window
   the selection gate reads in place of the two display fields `DeviceConstraints` held. Every other
   player reads it once at construction, exactly as before. It does not join rule 1's list, and no
   trigger joins rule 4's, because no policy reads the display yet. ADR-0014 rules 9 and 11 fix the
   property and the trigger, `PlaybackConditions.display` and `DISPLAY_CHANGED`, that the first
   reader adds in its own change, amending rules 1 and 4 then. The second row does not move: decoder
   profiles, levels, instance limits and the secure decoders are the device's own, no hotplug changes
   them, and they stay a constraint read once.
...
5. **A re-applied decision is honoured whole, or the policy is not re-consulted at all.** A changed
   ...
   *Addendum (2026-09-17, ADR-0014 rules 5 and 7).* Two exceptions, each narrow. First, on a player
   built with `superplayer-tv`, a display change rewrites the viewport in `TrackSelectionParameters`
   where it still holds the value Media3 derived from the display at construction. That value is a
   reading of the display rather than a consumer's parameter, and a viewport the consumer set is
   never rewritten. Second, the output half of a decision (tunneling) is applied from the
   construction decision only and never re-applied, because toggling it re-enables both renderers.
