...
public object LogcatSink : TelemetrySink {
    ...
    override fun onEvent(event: TelemetryEvent) {
        ...
        if (event is TelemetryEvent.StartupFailed || event is TelemetryEvent.MidStreamFailed) {
            Log.w(TAG, line)
        } else {
            Log.i(TAG, line)
        }
     ...
     */
    private fun format(event: TelemetryEvent): String {
        ...
        return when (event) {
            ...
            is TelemetryEvent.SessionEnded ->
                "evt=session_ended $common dropped=${event.droppedEventCount}"
            ...
            is TelemetryEvent.RebufferStarted ->
                "evt=rebuffer_started $common seekInduced=${event.seekInduced}"
            ...
            is TelemetryEvent.StartupFailed ->
                "evt=startup_failed $common ${failure(event.failure)}"
            ...
            is TelemetryEvent.LiveLatencySampled ->
                "evt=live_latency $common latencyMs=${event.liveLatencyMs} targetMs=${event.targetLiveLatencyMs}"
            ...
            is TelemetryEvent.VideoFramesDropped ->
                "evt=video_frames $common dropped=${event.droppedFrames} repeated=${event.repeatedFrames} " +
                    "elapsedPlayingMs=${event.elapsedPlayingMs}"
        }
    }

    private fun failure(failure: PlaybackFailure): String =
        "category=${failure.category} code=${failure.code} message=${quote(failure.message)}"
