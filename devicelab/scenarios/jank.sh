...
# A bound, not a duration: the scenario ends the trace itself, after about a minute.
...
# The feed's length, as the leak hunt passes it: the default, and far longer than the scroll reaches.
...
scenario_setup() {
    ...
    # The service's player is paused, as the leak hunt does before the feed: it is not the feed's, and a
    ...
    relaunch_demo --es com.superplayer.demo.extra.SCREEN FEED \
        --ei com.superplayer.demo.extra.FEED_ROWS "$JANK_FEED_ROWS"
    sleep 5
...
# Back to the feed's top after the warm-up, so the traced pass scrolls the same rows. Twice as many swipes
# as went down, since a swipe up the list flings no further than one down it, and surplus swipes at the
# top are no-ops.
jank_back_to_top() {
    local swipe=0 swipes=$((JANK_ROUNDS * JANK_SWIPES * 2))
