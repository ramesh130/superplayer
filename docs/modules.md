...
| `superplayer-abr` | 3 | BandwidthOracle (per-transport estimates, spread beside the mean, cache hits excluded); AdaptivePolicy — AdaptiveBufferPolicy's five branches over a retargetable AdaptiveLoadControl; NetworkAwareTrackSelection to follow | core, testkit (tests only) |
