...
devicelab/lab run jank [--plant P]     # the feed scrolled and tapped in one trace: frames, main thread, GC (devicelab/jank/README.md)
...
- **[ADR-0018](docs/adr/0018-push-encoded-frames-through-one-framesource-into-media3s-own-sample-queues.md)** —
  ...
  pair is needed either way and the renderer is pure addition. Container framing is stripped before
  the seam, so no fragmented-MP4 parsing exists here; codec-specific data is keyed on the codec's
  **fourcc** and never on the container; a realtime stream refuses `StartPosition.At` and
  `ResumeFromLastKnown` typed rather than coercing them; and a realtime URI is dispatched in
  `TransferChain` on its **scheme**. `superplayer-realtime` is core's **tenth** Kotlin friend, and
  the transport modules over it are deliberately **not** friends. What the path does **not** reach is
  a rule rather than a discovery — the cache, CMCD, downloads, the fallback ladder's load-error rungs
  and bandwidth estimation are all keyed to `DataSource`, and **ADR-0016's four chains do not become
  five**; what survives is `MediaRequest.sources` and rung 4. Adaptive selection is **deferred, not
  excluded** (rule 7), and a phase adding it amends the record. Telemetry keeps rebuffer and startup
  metrics, loses bandwidth samples, and reports **no upstream loss at all**, with `SCHEMA_VERSION`
  unmoved. Signalling does **not** travel `HttpTransport`. Nine of its thirteen rules are enforced by
  nothing in `check`, which the record says at each rule and then sums.
