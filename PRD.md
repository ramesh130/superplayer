...
| **10 — HTTP stack** (1–2 wks) | ADR-0004 implemented: `HttpStack` on the builder, an `HttpTransport` the consumer implements over whatever client the app already has, adapted to Media3 behind the boundary, and the platform's `HttpEngine` beside the default. One composition point for the playback, download, licence and doctor chains. | A consumer's own HTTP client carries all four chains; bandwidth estimation, CMCD, cache keying and every rung of the fallback ladder behave identically over it, each verified by a test rather than asserted in a comment; a stack that cannot be honoured fails at `build()` with a typed error rather than falling back silently. |
...
| **12 — Realtime spikes** (1–2 wks) | Timeboxed spikes before any production code: the MoQ stats surface, the byte format of a MoQ broadcast's codec description, MoQ's distribution and licence position, and — for the deferred WebRTC half — whether encoded frames are reachable at all and what a WHEP CDN actually offers. Nothing merges but the findings and one ADR. | ADR-0018 accepted, deciding how realtime frames reach the playback loop and what that path does **not** reach. Every spike ends in a written finding, including the ones that fail or that only half-answer. |
| **13 — Realtime source seam** (2–3 wks) | A public `FrameSource` a transport implements — encoded frames with timestamps, codec and optional codec-specific data, naming no Media3 type — in `superplayer-core`, so that its conformance suite can be `superplayer-testkit`'s (ADR-0018 rule 2's #356 addendum); and behind it, in `superplayer-realtime`, the push `MediaSource`/`MediaPeriod` writing into Media3 `SampleQueue`s under a live, unseekable timeline, the codec-string mapping, the codec-specific-data conversion and its in-band alternative, and a conformance suite in testkit. | A `FrameSource` fake plays synthetic H.264 and AAC in sync to `STATE_READY` with an advancing position under Robolectric, with no device and no network; `FrameSourceConformance` passes over the fake; a golden trace is committed. |
| **14 — MoQ** (3 wks) | `superplayer-moq` over Phase 13's seam: Media over QUIC through `moq-dev/moq`'s published Kotlin bindings, which hand over encoded frames with container framing already stripped. | A broadcast plays in the demo on a device; both codec-description shapes are covered by a test; session statistics are read through the bindings, and the loss statistics that are not exposed are filed upstream with a citation rather than silently absent. |
| **15 — WebRTC / WHEP** (3 wks, contingent) | `superplayer-whep`: WHEP signalling, the `PeerConnection` stack, and a `FrameSource` over it. **Deliberately after MoQ**, because its central question — whether encoded frames are reachable through the Android decoder seam at all — is unsettled in a way MoQ's never was, and because its signalling does not fit the HTTP seam this library already has. | A live WHEP stream plays audio and video in sync in the demo on a device; teardown releases the session; a latency figure is measured and committed. Entering this phase at all is conditional on Phase 12's WebRTC findings, which is why it is last. |
| **16 — Upstream and docs** (open-ended) | The half of this work that is not in this repository: the upstream contributions Phase 12 identified, and the adopter's documents for whichever extensions shipped. | The upstream contributions are filed with their evidence, and each shipped extension has a document answering an integrator who has never seen this repository. The exit deliberately stops at what is ours to control: a maintainer's response is not a criterion. |

Roughly five months at a sustainable part-time pace, before tuning. Phases 0–5 (about three months)
constitute a complete, coherent library on their own. Phases 12–16 are a second body of work rather
than a continuation of the first: they add transports, and nothing in phases 0–11 waits on them.

**Function before tuning.** Phases 0–10 end on behaviour that is built, tested and *measured*; none
ends on a number having moved in the right direction. Phases 12–16 keep that rule and run **beside**
Phase 11 rather than after it, since Phase 11 is open-ended and no transport waits on a benchmark;
the one number they do commit to, a WHEP latency figure, is a measurement reported rather than a
target to be met. Moving a benchmark number is iterative and
its outcome is not known in advance — a policy change fixes one cell and costs another, and each
attempt is a full matrix run — so it is Phase 11's alone, and no earlier phase waits on it. The
measurement is not deferred with it: the harness, the report and the QoE gate stay in the phases
that build them, so Phase 11 starts from committed reports rather than from nothing, and a
functional phase's report is a finding for Phase 11 rather than a reason to retune before
committing it.
...
1. **Measurement before tuning.** Telemetry and the benchmark harness are Phase 2, ahead of ABR in
   ...
   order puts tuning itself last: Phase 11 moves the numbers the earlier phases measure.
